...
namespace GamePushEditor
{
    public static class GP_ProductCatalogMapper
    {
        ...
        private static void AssignPrice(FetchProducts product, Dictionary<string, string> row, Platform platform)
        {
            ...
            {
                SetPrice(product, platformPrice, CurrencyForPlatform(platform));
                return;
            }

            if (TryGetPositive(row, "realPrices.RUB", out float rub))
            {
                SetPrice(product, rub, "RUB");
                return;
            }
            ...
            foreach (KeyValuePair<string, string> pair in row)
            {
                ...
                SetPrice(product, value, pair.Key.Substring("prices.".Length));
                return;
            }

            if (TryGetPositive(row, "basePrice", out float basePrice))
            {
                SetPrice(product, basePrice, FirstNonEmpty(ReadString(row, "baseCurrency"), "GP"));
                return;
            }

            if (TryGetPositive(row, "baseRealPrice", out float baseReal))
            {
                SetPrice(product, baseReal, FirstNonEmpty(ReadString(row, "baseRealCurrency"), "RUB"));
                return;
            }

            if (TryGetPositive(row, "price", out float flat))
                SetPrice(product, flat, FirstNonEmpty(ReadString(row, "currency"), "GP"));
        }

        private static void SetPrice(FetchProducts product, float value, string currency)
        {
            product.price = ToPriceInt(value);
            product.currency = currency;
            product.currencySymbol = currency == "RUB" ? "₽" : currency;
        }
        ...
        private static int ToPriceInt(float value)
        {
